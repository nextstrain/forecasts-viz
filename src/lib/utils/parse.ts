 ...
 *
 * Conceptual contents (accessed via `.get(...)`):
 * - `points` — hierarchical lookup `location → variant → ...`
 *   (see {@link parseModelData} for the layout).
 * - `variants` — model-defined variants (`modelJson.metadata.variants`).
 * - `dates` — sorted `YYYY-MM-DD` strings, guaranteed dense (no holes).
 *   Bridges `metadata.dates` and `metadata.forecast_dates`.
 * - `locations` — model-defined locations (`modelJson.metadata.location`).
 * - `dateIdx` — lookup `date string → index in dates`.
 * - `variantColors` — provided via `DatasetConfig`. Defaults applied otherwise.
 * - `variantDisplayNames` — provided via `DatasetConfig`. Falls back to keys.
 * - `pivot` — `metadata.pivot` if present, else the final entry in variants.
 * - `nowcastFinalDate` — crossover date between nowcasting and forecasting.
 * - `updated` — `metadata.updated` if present.
 * - `domains` — per-site `[min, max]` for non-temporal sites.
 */
// TODO check this — replace `any` with a per-key discriminated map once
// callers are TS-converted. The value type currently varies per key.
